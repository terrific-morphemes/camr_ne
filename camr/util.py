...
prep_list = ["aboard", "about", "above", "across", "after", "against", "along", "amid", "among", "anti", "around", "as", "at", "before", "behind", "below", "beneath", "beside", "besides", "beyond", "by", "concerning", "considering", "despite", "down", "during", "except", "excepting", "excluding", "following", "for", "from",
             "in", "inside", "into", "like", "minus", "near", "of", "off", "on", "onto", "opposite", "outside", "over", "past", "per", "plus", "regarding", "round", "save", "since", "than", "through", "to", "toward", "towards", "under", "underneath", "unlike", "until", "up", "upon", "versus", "via", "with", "within", "without"]
...
def lcsub(l1, l2, bt=False):
    '''
    longest common substring
    test doc: TODO add test cases here
    '''
    m = len(l1)
    n = len(l2)
    maxlen = 0
    maxendp = (0, 0)  # end points
    if bt:
        f = [[0 for j in range(n + 1)] for i in range(m + 1)]
        b1 = [i for i in range(m + 1)]
        b2 = [j for j in range(n + 1)]

        for i in range(1, m + 1):
            for j in range(1, n + 1):
                if l1[i - 1] == l2[j - 1]:
                    f[i][j] = f[i - 1][j - 1] + 1
                    if f[i][j] != 1:  # not update the start of substring
                        b1[i] = i - 1
                        b2[j] = j - 1
                else:
                    f[i][j] = 0
...
def get_unk_temlate(lex_form, tag_form):
    '''TODO: seperate the constrain with the actual functionality'''
    span1, span2 = lcsub(lex_form.lower(), tag_form.lower(), bt=True)
    placeholder = '.+'
    s1, e1 = span1
    if e1 - s1 > 3:
        abs_lex_form = '%s(%s)%s' % (lex_form[:s1], placeholder, lex_form[e1:])
        if abs_lex_form == '(%s)' % placeholder:
            return None
        s2, e2 = span2
        abs_tag_form = '%s(%s)%s' % (tag_form[:s2], placeholder, tag_form[e2:])
        return '%s->%s' % (abs_lex_form, abs_tag_form)
...
def is_abstract(node_idx, auto_align=False):
    if auto_align:
        return not isinstance(node_idx, int)
    else:
        return node_idx.startswith('a')
...
class ListMap(defaultdict):
    ...
    def __setitem__(self, k, v):
        if k in self:
            raise KeyError(
                'Cannot assign to ListMap entry; use replace() or append()')
        else:
            self._keys.append(k)
...
class Alphabet(object):
    ...
    def to_dict(self, index_to_label=False):
        if not index_to_label:
            new_table = dict([(str(key), value)
                              for key, value in self._label_to_index.items()])
        else:
            new_table = dict([(key, str(value))
                              for key, value in self._index_to_label.items()])
        return new_table
